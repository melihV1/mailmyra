...
| **photo-first** | ✅ avatar SÜTUNUNUN arkasına renk paneli, **YALNIZ gerçek fotoğraf varken** | "Yaratıcı vurgu" şablonu, en büyük avatar kutusu (88/104/120) onda. Koşul BİLEREK yalnız `hasAvatar` — `colspan` kararından (`hasAvatar \|\| shouldShowMonogram(data)`) FARKLI. ⚠️ **Revizyon (final review ①, kod bittikten sonra):** ilk sürümde koşul `colspan` ile AYNIYDI ("monogram zaten oraya renk koyuyor, panel onu çerçeveler" deniyordu) — YANLIŞ çıktı. Panel ile monogram AYNI `brand` hex'ini basıyordu; fotoğrafsız imzada monogramın disk silueti düz bir renk dikdörtgeninde kayboluyordu, ve Outlook Classic `border-radius`'ı da yok saydığı için orada hiç sınır kalmıyordu — bu, fotoğrafsız HER photo-first imzasının varsayılan hâliydi. Sahibinin kararı: monogram zaten bir renk bloğu, ikinci bir renk bloğu gereksiz — panel artık monogram varken ÇİZİLMEZ. **Sol hücre BOŞKEN de panel çizilmez** — ne avatar ne monogram varsa (adı boş bir imza) `leftCell` `''` olur ve renkli boş bir sütun saçma durur.  ⚠️ **İkinci revizyon (final review ② / 2026-09-15):** panel ilk sürümde sol hücrenin `padding-right`'ını da kaplıyordu (oluk marka rengine boyanıyor, metin renk bloğuna dayanıyordu); padding tamamen kaldırılınca bu sefer panel TAM fotoğrafın ayak izine indi (104px hücre, 104px görsel) ve Outlook Classic `border-radius`'ı yok saydığı için KISA imzalarda hiç görünmez oldu — yalnız sağ sütun avatardan uzunsa altta taşıyordu. Sahibinin kararı: **simetrik çerçeve**. Panel hücresi dört yönlü `padding: frame` alır (6/8/10 — card-bordered'ın bant ölçeğiyle aynı kademe, her boyutta `gap`in yarısı), `width` İÇERİK genişliği olarak (`avatar`) KALIR — `<td width>` content kutusuna düştüğü ve padding üstüne eklendiği için `avatar + 2×frame` yazmak content kutusunda boşluk bırakır, görsel sola yaslanır ve çerçeve sağda 3 kat kalın çıkar (bir turda tam bu hata yapıldı) — ve oluğun dış yarısı sağ hücreye `padding-left: gap − frame` olarak geçer. Böylece panel her boyutta görünür bir marka kenarı olur, oluğun dış yarısı boyanmaz ve avatar–metin mesafesi panel açık/kapalı AYNI kalır. ⚠️ **Üçüncü revizyon (Tur 4 / 2026-09-15, Apple Mail'de ölçüldü):** renk `<td>`'nin `bgcolor`'ı olduğu için panel SATIR YÜKSEKLİĞİ boyunca uzuyordu — uzun imzalarda kare tile olmaktan çıkıp tepesinde daire avatar olan tam boy bir marka sütununa dönüşüyordu. Sahibinin kararı: **yükseklik sınırlansın.** Panel artık avatarı saran bir İÇ TABLONUN hücresidir (`table()`'dan geçer, Outlook 2512 kenarlık düzeltmesini korur); iç tablo içeriğine göre daralır, satırla uzamaz, panel her imza uzunluğunda `avatar + 2×frame` kare kalır. Oluk tamamen sol hücreye taşındı (çerçeve iç yarı, `padding-right` dış yarı), sağ hücre hiçbir hâlde stil taşımaz. |
