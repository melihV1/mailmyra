...
`card-bordered`, `divider-columns`, `stacked-minimal` için, her birinin test dosyasına Task 4'teki `describe` bloğunun aynısını ekle — SADECE şu üçünü değiştir: `templateId`, beklenen medium kutu boyutu (`height="60"` / `height="90"` / `height="72"`), ve `renderSignature(..., '<şablon-adı>')`.

- [ ] **Step 2: Üç şablona dalı ekle**

Her birinde, Task 4'teki `else if (shouldShowMonogram(data)) { … }` bloğunun aynısını `if (data.visuals.avatarUrl)` bloğunun kapanışına ekle. `size: s.avatar`, `borderRadius: '4px'` aynı kalır. Import satırını da ekle.
...
`photo-first.ts:339` `const leftCell = hasAvatar ? cell(…) : '';` yapısında (else tarafı satır 356). Sondaki `: '';` yerine, Step 4'teki blokla AYNI yapıyı yaz — tek farkla:
...
for (const templateId of TEMPLATE_IDS) {
  describe(`monogram coverage: ${templateId}`, () => {
    ...
    it('never lets the monogram fall below 4.5:1 contrast', () => {
      // readableTextOn garantisi; burada sablon uzerinden dogrulanir.
      expect(html).toMatch(/color:#(000000|ffffff)/);
    });
  });
}
```
