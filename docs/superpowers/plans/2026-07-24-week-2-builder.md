...
3. **İçerik çakışması artık reddediliyor (semantik değişikliği, ONAYLANMIŞ anayasa değişikliği — bkz. Task 5 notu ve CLAUDE.md "Görsel Boyut Politikası").** `save()` EEXIST aldığında önceden mevcut içeriği koşulsuz koruyup başarı dönüyordu — aynı dosya adında FARKLI bir içerik görülmesi (hash fonksiyonunda regresyon, elle dosya yerleştirme) sessizce yutuluyordu. Artık EEXIST durumunda disk üzerindeki içerik `readFile` ile okunup gelen buffer'la karşılaştırılıyor: aynıysa (idempotent tekrar/eşzamanlı aynı yükleme) başarı; FARKLIYSA `Error("Content collision for immutable file: <filename>")` fırlatılıyor, dosya üzerine yazılmıyor. `apps/web/test/storage.test.ts`'teki `'never overwrites an existing file (immutability)'` testi bu yeni semantiğe göre güncellendi (artık reddi ve değişmeyen içeriği doğruluyor); TOCTOU açıklayan iki test de (tek dosya + eşzamanlı yazma) gerçekçi senaryoya (içerik-adresli isimlendirmede eşzamanlı yazarların içeriği her zaman AYNIDIR) uyacak şekilde güncellendi. Yeni testler: farklı içerikte ret, aynı içerikte başarı.
(Bu düzeltmelerin gerçek commit'i İKİ commit'e bölünmüş durumda — madde 1-2 `fix(web): harden upload route...`'ta, madde 3 `feat(web)!: 16-hex content hashes with collision detection`'da; bkz. dosya sonundaki özet.)
...
**Kod incelemesi sonrası düzeltme (final review dalgası, 2026-07-24 — ONAYLANMIŞ anayasa değişikliği, bkz. CLAUDE.md "Görsel Boyut Politikası"):**
5. **Hash uzunluğu 8 → 16 hex.** `createHash('sha256')...slice(0, 8)` yerine `slice(0, 16)`. Gerekçe: `storage.ts` artık aynı dosya adında farklı içerik gördüğünde reddediyor (bkz. Task 4 notu) — bu bir çarpışmayı sessiz veri bozulması olmaktan çıkarıp açık bir hataya çeviriyor, ama 32 bitlik (8 hex) alan, üretim ölçeğinde meşru bir yüklemenin bu hataya çarpma ihtimalini gereksiz yere yükseltiyordu. 64 bitlik (16 hex) alan bu riski pratikte ortadan kaldırıyor. `apps/web/test/image-pipeline.test.ts` ve `apps/web/test/upload-route.test.ts` içindeki `/^[0-9a-f]{8}\.../` regex'leri `{16}`'ya güncellendi.
