...
describe('stacked-minimal monogram', () => {
  ...
  const noPhoto: SignatureData = {
    ...
    social: [],
    layout: { templateId: 'stacked-minimal', size: 'medium' as const, iconStyle: 'mono' as const, showDividers: false },
  };

  it('shows a monogram when there is no photo', () => {
    const html = renderSignature(noPhoto, 'stacked-minimal');
  ...
  it('uses this template medium avatar box', () => {
    expect(renderSignature(noPhoto, 'stacked-minimal')).toContain('height="72"');
  });
});

describe('stacked-minimal name spacing', () => {
  const base: SignatureData = {
    ...
    social: [],
    layout: { templateId: 'stacked-minimal', size: 'medium', iconStyle: 'mono', showDividers: false },
  };

  it('adds no tracking by default', () => {
    expect(renderSignature(base, 'stacked-minimal')).not.toContain('letter-spacing:0.04em');
  });
  it('opens the tracking when asked', () => {
    expect(
      renderSignature({ ...base, layout: { ...base.layout, nameSpacing: 'wide' } }, 'stacked-minimal'),
  ...
  it('never changes the letters of the name', () => {
    for (const ns of [undefined, 'normal', 'wide'] as const) {
      const html = renderSignature({ ...base, layout: { ...base.layout, nameSpacing: ns } }, 'stacked-minimal');
