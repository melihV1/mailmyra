...
export function dividerColumns(data: SignatureData, opts?: RenderOptions): string {
  ...
  if (data.visuals.avatarUrl) {
    ...
    // avatarıyla AYNI yarıçap (4px) — yeni bir tutarsızlık getirmez. Boşluk
    // kararı avatarUrl'e DEĞİL logoUrl'e bağlı (yukarıdaki satırla aynı
    // koşul) — monogram da logonun altına düşünce aynı payı almalı.
    visualRows.push(
      row(
        cell(
          monogramCell({
