...
function LeadCard({ lead, tone, preview }: { lead: GrowthLeadRow; tone: string; preview?: boolean }) {
...
export function LeadsView({ rows, preview }: { rows: GrowthLeadRow[]; preview?: boolean }) {
  ...
  return <>
    ...
    {rows.length ? <>
      ...
      <div className="mm-lead-board mb-6">{groups.map((group) => { const meta = LEAD_META[group.stage]; const seats = group.rows.reduce((sum, row) => sum + row.seats, 0); return <section className={`mm-lead-column mm-lead-column--${meta.tone}`} key={group.stage}><header className="mm-lead-column__header"><div className="d-flex align-items-center gap-3"><span className="avatar avatar-sm"><span className={`avatar-initial rounded bg-label-${meta.tone} text-${meta.tone}`}><i className={`icon-base ti ${meta.icon}`} /></span></span><span><span className="d-flex align-items-center gap-2"><h6 className="mb-0">{meta.label}</h6><span className={`badge rounded-pill bg-label-${meta.tone}`}>{group.rows.length}</span></span><small className="text-body-secondary">{seats} potential seats</small></span></div></header><div className="mm-lead-column__progress"><span className={`bg-${meta.tone}`} style={{ width: `${meta.step}%` }} /></div><div className="d-grid gap-3">{group.rows.map((lead) => <LeadCard lead={lead} tone={meta.tone} preview={preview} key={lead.id} />)}</div></section>; })}</div>
    </> : <div className="row g-6 mb-6"><div className="col-xl-7"><div className="card h-100"><div className="card-body py-10 text-center"><span className="avatar avatar-xl mb-4"><span className="avatar-initial rounded bg-label-primary"><i className="icon-base ti tabler-user-search icon-32px" /></span></span><h4>No leads yet</h4><p className="text-body-secondary mx-auto mb-0" style={{ maxWidth: '42rem' }}>Leads opened from this board will appear here, grouped by pipeline stage.</p></div></div></div><div className="col-xl-5"><div className="card h-100 card-border-shadow-primary"><div className="card-body"><OperationsSectionHeader title="Pipeline fields" support="What a lead record carries." /><div className="d-grid gap-3">{['Company and contact', 'Source', 'Seat estimate', 'Pipeline stage', 'Next step'].map((label, index) => <div className="d-flex align-items-center gap-3" key={label}><span className="badge rounded-pill bg-label-primary">0{index + 1}</span><span className="text-heading fw-medium">{label}</span></div>)}</div></div></div></div></div>}
    <GrowthSource preview={preview} body={preview ? 'The board demonstrates the intended CRM workflow and does not represent production leads.' : 'Leads are a manually curated, writable register — there is no automatic CRM or tracking source. Stage, seats and next step change only through this board.'} />
  </>;
}
