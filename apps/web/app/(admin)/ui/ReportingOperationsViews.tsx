...
export function ScheduledReportsView({ rows, now, preview = false }: { rows: ReportSchedule[]; now: number; preview?: boolean }) {
  ...
  if (!rows.length) return <>
    <OperationsKpiStrip><OperationsKpi label="Schedules" value="0" support="No persisted scheduler records" icon="tabler-calendar-off" tone="secondary" /><OperationsKpi label="Delivery channel" value="Not connected" support="Email/export worker required" icon="tabler-send-off" tone="warning" /><OperationsKpi label="Run history" value="Unavailable" support="No durable execution ledger" icon="tabler-history-off" tone="danger" /><OperationsKpi label="Definitions" value="Ready" support="Use the report library" icon="tabler-report-analytics" tone="primary" last /></OperationsKpiStrip>
    <div className="card"><div className="card-body py-10"><div className="mm-report-empty"><span className="avatar avatar-xl"><span className="avatar-initial rounded bg-label-primary"><i className="icon-base ti tabler-calendar-cog icon-32px" /></span></span><div><h4>Scheduled delivery needs persistence</h4><p className="text-body-secondary mb-0">Add schedule, recipient, format, next-run and immutable execution records before enabling create, pause or retry controls.</p></div></div></div></div>
    <div className="mt-6"><SourceNotice title="No simulated delivery in production" body="Preview routes demonstrate the intended scheduler UI; production does not claim that reports were sent." tone="warning" icon="tabler-database-off" /></div>
  </>;
  return <>
    ...
    <div className="card mm-report-scheduler">
      <div className="card-body"><OperationsSectionHeader title="Delivery calendar" support="Cadence, owner, recipients and execution state without hiding failed runs." action={<button className="btn btn-primary" type="button" disabled={!preview}><i className="icon-base ti tabler-plus me-2" />New schedule</button>} />
    ...
    {selected && <StaffDialog title={selected.reportName} subtitle="Schedule definition" labelledBy={selected.reportName} busy={false} onClose={() => setSelected(null)}><div className="list-group"><Detail label="Cadence" value={selected.cadence} /><Detail label="Format" value={selected.format} /><Detail label="Owner" value={selected.owner} /><Detail label="Recipients" value={selected.recipients.join(', ')} /><Detail label="Next run" value={formatCompactDate(selected.nextRunAt)} /><Detail label="Last status" value={selected.lastRunStatus ?? 'Never run'} /></div></StaffDialog>}
  </>;
}
