...
function ReleaseCard({ row, t, featured = false }: { row: ReleaseRow; t: AdminPlatformDict; featured?: boolean }) {
...
export function FeatureFlagsView({ source, preview }: { source: PlatformOperationsSnapshot; preview?: boolean }) {
  ...
  return <>
    ...
    {source.telemetry.flags.length ? <><div className="alert alert-danger d-flex align-items-start gap-3 mb-6"><i className="icon-base ti tabler-shield-lock icon-24px mt-1" /><div><h6 className="alert-heading mb-1">{t.flags.highImpact.title}</h6><p className="mb-0">{t.flags.highImpact.body}</p></div></div><div className="mm-flag-grid mb-6">{source.telemetry.flags.map((row) => <FlagCard row={row} preview={preview} t={t} key={row.id} />)}</div></> : <div className="card mb-6"><div className="card-body"><EmptyTelemetry icon="tabler-flag-off" title={t.flags.empty.title} body={t.flags.empty.body} /></div></div>}
