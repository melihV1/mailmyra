...
export function DataRequestsView({ rows, now, preview = false }: { rows: DataRequestRow[]; now: number; preview?: boolean }) {
  ...
  return <>
    ...
    {selected && !action && <StaffDialog title={selected.reference} subtitle={preview ? 'Preview-only request detail.' : 'Request detail.'} labelledBy={selected.reference} busy={false} onClose={() => setSelected(null)} wide><div className="row g-4 mb-6"><div className="col-sm-6"><Row label="Subject" value={selected.subjectEmail} /><Row label="Customer" value={selected.customer} /><Row label="Owner" value={selected.owner ?? 'Unassigned'} /></div><div className="col-sm-6"><Row label="Request type" value={selected.type} /><Row label="Received" value={formatCompactDate(selected.receivedAt)} /><Row label="Due" value={formatCompactDate(selected.dueAt)} /></div></div>{!preview && <div className="d-flex flex-wrap gap-2 mb-6"><KvkkActionButtons row={selected} onPick={setAction} /></div>}<button type="button" className="btn btn-label-secondary w-100" onClick={() => setSelected(null)}>{preview ? 'Close preview' : 'Close'}</button></StaffDialog>}
  </>;
}
