...
export function ApprovalsView({ rows, preview = false }: { rows: ApprovalQueueRow[]; preview?: boolean }) {
  ...
  return <>
    ...
    {selected && <StaffDialog title="Approval request" subtitle={preview ? 'Preview-only request context.' : 'Request detail.'} labelledBy={selected.title} busy={false} onClose={() => setSelected(null)}><div className="list-group mb-6"><Row label="Request" value={selected.title} /><Row label="Domain" value={selected.domain} /><Row label="Requester" value={selected.requester} /><Row label="Customer" value={selected.customer ?? 'Platform'} /><Row label="Decision progress" value={`${selected.approvals}/${selected.requiredApprovals}`} /></div>{!preview && selected.status === 'pending' && <div className="d-flex gap-2 mb-6"><ApprovalRowActions row={selected} onDone={() => setSelected(null)} /></div>}<button type="button" className="btn btn-label-secondary w-100" onClick={() => setSelected(null)}>{preview ? 'Close preview' : 'Close'}</button></StaffDialog>}
  </>;
}
